...
<button type="button" onclick="doOK()" id="uploadBtn" class="btn btn-primary" style="float:right;"><i class="glyphicon glyphicon-upload"></i> 审核通过</button>
<br>
 <hr style="clear:both;">
 		<#list mcs as mc>
 			<h1>${mc.certname }</h1>
 			<img alt="" src="${APP_PATH }/img/cert/${mc.iconpath}">
 		</#list>
			  </div>
			</div>
        </div>
	...
	<script src="/layer/layer.js"></script>
        <script type="text/javascript">
            ...
            function doOK(){
            	$.ajax({
            		type : "POST",
            		url : "${APP_PATH}/auth/finishAuth",
            		data : {
            			"memberid" : "${memberid}",
            			"taskid" : "${taskid}"
            		},
            		success : function(){
            			location="${APP_PATH}/auth/index";
            		}
            	});
            }
        </script>
