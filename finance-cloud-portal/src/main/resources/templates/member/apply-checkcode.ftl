 ...
 <div class="navbar-wrapper">
      ...
      <div class="page-header">
		...
		<form role="form" style="margin-top:20px;">
		  <div class="form-group">
			...
			<input type="text" class="form-control" id="checkcode" placeholder="请输入你邮箱中接收到的验证码">
		  </div>
          <button type="button" onclick="javascript:;" class="btn btn-primary">重新发送验证码</button>
		  <button type="button" onclick="finishApply()"  class="btn btn-success">申请完成</button>
		</form>
		<hr>
    </div> <!-- /container -->
	...
	<script>
        ...
        function finishApply(){
        	$.ajax({
        		type : "POST",
        		url : "${APP_PATH}/member/finishApply",
        		data : {
        			authcode : $("#checkcode").val()
        		},
        		success : function(){
        			location="${APP_PATH}/main"
        		}
        	});
        }
	</script>
